...
public class WindBurstProjectile : Projectile
{
    ...
    private void ApplyTickEffect()
    {
        ...
        if (ownerEntity == null)
        {
            Debug.LogError($"[WindBurst] ApplyTickEffect - owner not found for ID={ownerId}");
            return;
        }
        ...
        foreach (Collider c in hitColliders)
        {
            ...
            {
                Debug.Log($"[WindBurst] Tick - Hit enemy {e.name} for {damage} damage!");
        ...
        if (enemiesHit > 0)
        {
            Debug.Log($"[WindBurst] Tick complete - Hit {enemiesHit} enemies");
        }
    }
    ...
    private void DestroyProjectile()
    {
        ...
        isProcessing = false;
        Debug.Log($"[WindBurst] Duration ended ({duration}s) - destroying projectile");
